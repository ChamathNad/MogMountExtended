...
local function InitSettings()
	...
	for t = 1, #C_TransmogOutfitInfo.GetOutfitsInfo() do
		...
		-- Aquatic per outfit (new)

		local variable = "Aquatic"..outfitInfo.outfitID;
		local defaultValue = 1;
		local name = L["Settings Aquatic Mount"] or "Aquatic Mount";
		local tooltip = false;
		local variableKey = "Aquatic";
		local variableTable = MogMountCharacterSaved["Outfit"..outfitInfo.outfitID];

		local function GetOptionsAquaticMountSettings()
			...
			for i = 1, #mounts do
				local mount = mounts[i];
				container:Add(mount.id, mount.nameAndIcon);
				if mount.id == MogMountCharacterSaved["Outfit"..outfitInfo.outfitID].Aquatic then
					defaultValue = i;
				end
			end
		...
		local setting = Settings.RegisterAddOnSetting(category, variable, variableKey, variableTable, type(defaultValue), name, defaultValue);
		Settings.CreateDropdown(category, setting, GetOptionsAquaticMountSettings, tooltip);
		setting:SetValueChangedCallback(OnSettingChanged);
